var calendar = {
            ...
            for (var j = 0; j < 7; j++) {
                if (i == 0) {
                    if (firstDay > j) {
                        ...
                        var _dx = (y + "-" + m + "-" + _d);
                    	var testData = new Date(_dx.replace(/[-]/g, "/")).format("yyyyMMdd");
                        if (m == currMon && currDay == _d && currYear == y) {
                        	...
                        	calDay.push("<td date='" + _dx + "' class=' day_"+testData+"'>" + _d + "</td>");
                        }
                        _d++;
                    }
                }
                    ...
                    if (_d <= lastDay) {
                    	...
                    	var testData = new Date(_dx.replace(/[-]/g, "/")).format("yyyyMMdd");
                        if (m == currMon && currDay == _d && currYear == y) {
                            calDay.push("<td date='" + _dx + "' class='_selDay day_"+testData+"'>" + _d + "</td>");
                        }
                        else if (m == setMon && setDay == _d && setYear == y) {
                            calDay.push("<td date='" + _dx + "' title='" + _d + "号' class='_sday'>今天</td>");
                        }
                            ...
                            calDay.push("<td date='' class='disDay songlin1'>" + _d + "</td>");
                        }
                        else if (calendar.config.maxDay != null && new Date(calendar.config.maxDay.replace(/[年月日-]/g, "\/")) < new Date(_dx.replace(/[-]/g, "\/"))) {
                            calDay.push("<td date='' class='disDay songlin2'>" + _d + "</td>");
                        }
                        else {
                            calDay.push("<td  date='" + _dx + "' class='day_"+testData+"'>" + _d + "</td>");
                        }
    ...
    init: function (s) {
       ...
       // $("#_tdCal tr:gt(0)").delegate("td[date!='']","click",function (e) {
        $("#_tdCal tr:gt(0) td[date!='']").live("click",function(e){
	        	e.stopPropagation();
            s.value = new Date($(this).attr("date").replace(/[-]/g, "/")).format(calendar.config.format);
            sstime = new Date($(this).attr("date").replace(/[-]/g, "/")).format("yyyyMMdd");
            $(s).attr("date", sstime );
            if (calendar.config.ok != null) {
                eval(calendar.config.ok(sstime));
            }
            //默认点击完之后不关闭 2015-06-09注释
            ////calendar.colse();
        });
        $("#_tdCal tr:gt(0) td[date!='']").live("mouseover", function () {
