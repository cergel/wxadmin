...
class UploadFiles
{
     ...
     * @author liulong
     */
    public static function moveFile($filePath, $newPath, $overWrite = TRUE) {
        if (!is_file($filePath)) return false;
        if ($filePath == $newPath) return true;
        if (is_file($newPath) && $overWrite === FALSE) {
            return false;
        } elseif (is_file($newPath) && $overWrite === true) {
            UploadFiles :: unlinkFile($newPath);
        }
        UploadFiles::createPath(dirname($newPath));
     ...
     * @author liulong
     */
	public static function copyFile($filePath, $newPath, $overWrite = TRUE) {
        if (!is_file($filePath)) {
            return false;
        }
        if (is_file($newPath) && $overWrite === FALSE) {
        ...
        return is_file($newPath)?true:false;
    }
...
//     	}
	...
	public static function unlinkFileDirIndo($dirName){
		if(is_dir($dirName)){
			$dh = opendir($dirName);
			while($file = readdir($dh)){
				if($file != '.' && $file != '..'){
					...
					if(!is_dir($fullPath)){
						UploadFiles::unlinkFile($fullPath);
					}else{
						UploadFiles::unlinkFileDirIndo($fullPath);
					}
				}
			}
			closedir($dh);
    ...
    /**
     * @tutorial 创建目录 如果目录存在，则不创建，不存在则创建 static
     * @param $path 路径
     * @return 
     * @author liulong
     */
    public static function createPath($path) {
    	if (!is_dir($path)) {
    		UploadFiles::createPath(dirname($path));
    		@mkdir($path);
    		@chmod($path, 0777);
    	}
    }
    ...
    /**
     * @tutorial 修改图片类型-jpeg
     * @param unknown $imgPath
     * @param unknown $newImg
     */
    public static function editImg2Jpeg($imgPath,$newImg)
    {
    	$image=imagecreatefromwbmp($imgPath);
    	imagejpeg($imgPath,$newImg);
    	imagedestroy($image);
    	UploadFiles::unlinkFile($imgPath);
    }
	...
	public static function make_thumb($source, $newname, $width = 100, $height = 100, $isauto = true) {
		if (!is_file($source)) return false;
		$image_info = UploadFiles::get_img_info($source); //获取原始图片信息
        if ($image_info['type'] == 'bmp') return false;
		list($imagecreate, $imagecopyre) = UploadFiles::get_image_create($image_info['type']); //获取创建图像和拷贝图像的函数
		...
		$result = UploadFiles::make($image_info['type'], $thumb, $newname);
	...
	public function water_mark($source_path, $water_path, $postion, $pct=100, $quality=100) {
		...
		/* 图片信息 */
		$source_info = UploadFiles::get_img_info($source_path);
		$water_info  = UploadFiles::get_img_info($water_path);
		...
		list($x, $y) = UploadFiles::get_water_postion($postion, $source_info['width'], $source_info['height'], $water_info['width'], $water_info['height']);
		...
		UploadFiles::make($source_info['type'], $source, $source_path, $quality);
		imagedestroy($source);
		imagedestroy($water);
		return true;
	}
	 ...
	 * @return bool
	 */
	public static function make($type, $image, $filename, $quality = '75') {
		$file_type = ($type !== 'jpeg') ? $type : 'jpg';
		if (!strpos($filename,'.jp')){
			$filename = $filename.'.jpg';
		}
		UploadFiles::createPath(dirname($filename)); //创建目录 
	 ...
	 */
	public static function get_img_info($source) {
		$imginfo = array();
	 ...
	 */
	public static function get_image_create($imagetype) {
