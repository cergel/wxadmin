...
class redisOperator
{
    ...
    public static function getInstance($arrRedisConf)
    {
        if (!empty($arrRedisConf)) {
            $objKey = self::getObjKey($arrRedisConf) . '_' . strval(getmypid());
     ...
     * @return \Redis
     */
    public function WYMulti($strKey)
    {
    ...
    //利用sUnionStore将一个set存储到另一个set中
    public function WYsUnionStore($desKey, $srcKey, $srcKey1)
    {
        return $this->redis->sUnionStore($desKey, $srcKey, $srcKey1);
    }

    //删除指的score
    public function WYzRemRangeByScore($key, $scoreMin, $scoreMax)
    {
