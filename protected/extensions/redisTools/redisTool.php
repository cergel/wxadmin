...
Yii::import("ext.redisTools.redisOperator");
Yii::import("ext.redisTools.route.defaultRouter.defaultRouter");
Yii::import("ext.redisTools.route.moduloRouter.moduloRouter");
...
class redisTool
{
     ...
     */
    public static function getInstance($arrConfig = array(),$channelId = 'common')
    {

        if(empty($arrConfig[$channelId])){
            throw new redisManagerException("redis config is empty");
        }else{
            $arrConfig = $arrConfig[$channelId];
        }
        $objKey = md5(json_encode($arrConfig)) . '_' . strval(getmypid());
        $type = $arrConfig['type'];
    ...
    public function __call($func, $params)
    {
        ...
        try{
            ...
            try {
                ...
                $return = null;
                throw new redisManagerException("Redis server can not connect!" . json_encode($logInfo));
            }
        }
