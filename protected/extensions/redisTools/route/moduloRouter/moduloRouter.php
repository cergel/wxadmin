<?php
//namespace redisTools\route\moduloRouter;
require_once BASE_PRO_URL.'/extensions/redisTools/route/AbstractRouter.php';
 ...
 */
class moduloRouter extends AbstractRouter
{
     ...
     * @throws \Exception
     */
    public static function moduloConf($strKey, $rediscConf)
    {
        if (count($rediscConf) == 1) {//如果只有一个元素的话，不用一致性哈希算法
            return $rediscConf[0];
        }
        else {
            $num = abs(crc32($strKey));
            $countConf = count($rediscConf);
            $key = $num % $countConf;
            return $rediscConf[$key];
        }
    }
    ...
    public function getFinalConf($fun, $params)
    {
        ...
        if ($this->useSubKeyOnce === true) {
            ...
            if (in_array($fun, $enableFun)) {
                $moduloConf = self::moduloConf($params[1], $this->redisConf['db']);
        ...
        elseif ($this->useSubKeyStable === true) {
            ...
            $moduloConf = self::moduloConf($params[0], $this->redisConf['db']);
        }
        ...
        if ($this->onceType) {//如果有设置一次的
            if ($this->onceType == 'write') {
                $finalConf = $moduloConf['write'];
            }
            else {
                $finalConf = $moduloConf['read'];
            }
            $this->setDefaultOnce();
        }
