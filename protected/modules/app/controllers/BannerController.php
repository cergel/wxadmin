		...
		if(isset($_POST['Banner']))
		{
			...
			if($model->save()) {
                ...
                if($sSharePic) {
                	...
                	$uploadDirSharePic = Yii::app()->basePath . '/../uploads/app_banner/' . date('Y-m-d', $model->iCreated);
                	if (!file_exists($uploadDirSharePic))
                		mkdir($uploadDirSharePic, 0777, true);
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Banner']))
		{
			...
			if($model->save()) {
                ...
                //处理分享图标
                if($sSharePic) {
                	$model->sSharePic='sSharePic'.$model->iBannerID.rand(1000, 9999).'.'.$sSharePic->getExtensionName();
                	$uploadDirSharePic = Yii::app()->basePath . '/../uploads/app_banner/' . date('Y-m-d', $model->iCreated);
                	if (!file_exists($uploadDirSharePic))
                		mkdir($uploadDirSharePic, 0777, true);
                	$sSharePic->saveAs($uploadDirSharePic . '/' . $model->sSharePic, true);
