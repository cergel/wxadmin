 ...
 */
class ActivePage extends CActiveRecord
{
    ...
    {
        return array(
            array('iAppWill,sStyle,iChannel,iType,iTime,iMovieId,sContent, sExtend, sNotice,iwx,iqq,imobile,iPreheatEndTime,iEndTime,sDirector,sDirectorPhone,iShowStartTime,iShowEndTime', 'safe'),
            array('iShowEndTime,iShowStartTime,iMovieId,sName, sTitle, sRule, sShareTitle, sShareContent,iPreheatEndTime,iEndTime,sDirector,sDirectorPhone', 'required'),
            array('iAppWill,iDeleted,iMovieId, iCreated, iUpdated,iPreheatEndTime,iEndTime', 'numerical', 'integerOnly' => true),

            array('sName, sTitle, sButtonText', 'length', 'max' => 100),
            array('sFooterText, sFooterLink', 'length', 'max' => 200),
            array('sDirector','length','max'=>10),
//            // 下面这种写法是为了防止不更新图片的时候value被validator清空
//            // 原因不明，还需要研究，暂时这样解决
//            CUploadedFile::getInstance($this, 'sPic') || $this->isNewRecord ? array(
//                'sPic',
//                'file',
...
//            ) : array('sPic', 'length', 'max' => 200),
//            CUploadedFile::getInstance($this, 'sSharePic') || $this->isNewRecord ? array(
//                'sSharePic',
//                'file',
//                'allowEmpty' => !$this->isNewRecord,
//                //'types' => 'jpg,png,gif',
...
//            ) : array('sSharePic', 'length', 'max' => 200),
//            array('sFooterLink', 'url'),
//            // The following sRule is used by search().
//            // @todo Please remove those attributes that should not be searched.
            array(
                'iAppWill,sStyle,iType, iChannel,sExtend, sNotice,iTime, iActivePageID, sName, sTitle, sRule, iActiveID, sShareTitle, sShareContent, sStyle, sExtend, iDeleted, iCreated, iUpdated,iwx,iqq,imobile,iPreheatEndTime,iEndTime,sDirector,sDirectorPhone',
                'safe',
                'on' => 'search'
            ),
        );
...
//            'sTempurl'=>'分享链接',
            ...
            'iChannel'=>'创建渠道',
            'sDirector'=>'负责人',
            'sDirectorPhone'=>'负责人电话',
            'sUpdatedName'=>'最后更新人',
            'iShowStartTime'=>'前台展示开始时间',
            'iShowEndTime'=>'前台展示结束时间',
            'iAppWill'=>'客户端提前抢',
        );
    }
    ...
    public function search()
    {
        ...
        $criteria->compare('iAppWill',$this->iAppWill);
        ...
        $criteria->compare('sDirector', $this->sDirector, true);
        $criteria->compare('sDirectorPhone',$this->sDirectorPhone);
    ...
    public function beforeSave()
    {
        ...
        $this->sUpdatedName = $userModel->sUserName;
        $this->iUpdated = time();
        if ($this->isNewRecord) {
            $this->iCreated = time();
        }
    ...
    public function getDialogInfo($id,$iType,$wx,$qq,$mobile)
    {
        ...
        $str ="";
            ...
            $str .="&nbsp;&nbsp;&nbsp;&nbsp;本地测试链接:<a target ='_blank' href=\"".Yii::app()->params['active_page_new']['target_url']."$type/$id/index.html\">";
            $str .=Yii::app()->params['active_page_new']['target_url']."$type/$id/index.html</a></br>";
            $str .="&nbsp;&nbsp;&nbsp;&nbsp;线上发布链接：<a target ='_blank' href=\"".Yii::app()->params['active_page_new']['final_url']."$type/$id/index.html\">";
            $str .=Yii::app()->params['active_page_new']['final_url']."$type/$id/index.html</a>";
    ...
    // 生成模板文件:单影片
    public function makeFile($active ='active_page_new')
    {
        $json ='';
        $channelType = '';
...
//                $bson = BsonBaseCinema::model()->findByAttributes(array('CinemaNo' => $value->iCinemaID));
        ...
        $activeTime = date('m.d',$this->iTime +1).'-'.date('m.d',$this->iEndTime);
        //写入html
